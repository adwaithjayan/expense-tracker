  ...
  return (
    <View style={styles.container}>
        ...
        <View style={styles.list}>
            <FlashList data={data} renderItem={({item,index})=><TransactionItem item={item} index={index} handleClick={handleClick}/>} estimatedItemSize={50}/>
        </View>
        {!loading && data.length > 0 && <Typo size={15} color={colors.neutral400} style={{textAlign:'center',marginTop:spacingY._15}}>{emptyListMessage}</Typo>}
    ...
    return <Animated.View entering={FadeInDown.delay(index*70).springify().damping(14)}>
        <TouchableOpacity style={styles.row} onPress={()=>handleClick(item)}>
            ...
            <View style={styles.amountDate}>
                ...
                <Typo  color={colors.neutral400} size={13}>{date}</Typo>
            </View>
        </TouchableOpacity>
    </Animated.View>
}
